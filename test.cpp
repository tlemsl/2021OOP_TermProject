...
protected:
	void End_session(){
		cout<<BOLDGREEN<<"현재 세션이 종료되었습니다. 감사합니다."<<RESET<<endl;
	}
	void End_session_message(){
	...
	void Wrong_cardnumber(){
		cout<<BOLDRED<<"잘못된 카드 번호를 입력하셨습니다."<<RESET<<endl;
	}
	void Wrong_passwords(){
		cout<<BOLDRED<<"잘봇된 비밀번호를 입력하셨습니다."<<RESET<<endl;
	}
	void Lack_of_balance(){
		cout<<BOLDRED<<"잔액이 부족합니다."<<RESET<<endl;
	}
	void Wrong_account_number(){
		cout<<BOLDRED<<"잘못된 계좌번호를 입력학셨습니다."<<RESET<<endl;
	}
	void Over_max_atm_input(){
		cout<<BOLDRED<<"ATM이 받아 들일 수 있는 최대 금액을 초과했습니다."<<RESET<<endl;
	}
	void Session_display(vector<string>& log){
